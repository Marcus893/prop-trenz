...
function PricingCard({ plan, currency, isPopular, onSelect, loadingPlan, currentPlan, isExpired }: PricingCardProps) {
  ...
  const planConfig = {
    ...
    yearly: {
      ...
      badge: t('subscription.save_percent', 'Save {{percent}}%', { percent: getYearlyDiscount() }) as string | null,
      features: [
        t('subscription.feature_unlimited', 'Unlimited transactions'),
        t('subscription.feature_checklists', 'Stage checklists & tracking'),
        t('subscription.feature_costs', 'Cost tracking & estimates'),
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-6">
        <div className={`inline-flex items-center justify-center w-12 h-12 rounded-full ${colors.icon} mb-3`}>
...
export function UpgradeModal({ isOpen, onClose, reason = 'general' }: UpgradeModalProps) {
  ...
  const [currency, setCurrency] = useState<Currency>(subscription?.preferredCurrency || 'MXN')
  ...
  const reasonMessages = {
    limit_reached: t(
      ...
      "You've reached the free tier limit of 1 transaction. Upgrade to track unlimited transactions."
    ),
    feature_locked: t(
        ...
        <div className="inline-block w-full max-w-4xl my-8 text-left align-middle transition-all transform bg-white shadow-2xl rounded-2xl relative">
          ...
          <div className="px-8 pb-8 text-center">
            <p className="text-sm text-gray-500">
              {t('subscription.secure_payment', 'Secure payment powered by Stripe. Cancel anytime.')}
            </p>
          </div>
        </div>
      </div>
...
export function UpgradeModalWithContext() {
  ...
  return <UpgradeModal isOpen={isUpgradeModalOpen} onClose={closeUpgradeModal} reason="limit_reached" />
}
