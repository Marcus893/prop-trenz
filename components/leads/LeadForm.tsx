...
function normalizeCity(city: string | undefined): string {
  ...
  if (normalized.toLowerCase().includes('ciudad de méxico') || 
  ...
  if (normalized.toLowerCase().includes('guadalajara') ||
...
export function LeadForm({ isOpen, onClose, source, context = {} }: LeadFormProps) {
  ...
  const getTranslation = (key: string, defaultValue: string) => {
    if (source === 'location_page') {
      ...
      const esTranslations: Record<string, string> = {
        ...
        'leads.form_description': 'Conéctate con un profesional inmobiliario verificado en tu área.',
        ...
        'leads.location_interest_heading': '¿En qué ubicación estás interesado?',
        ...
        'leads.consent_text': 'Al enviar, aceptas ser contactado por nuestros profesionales inmobiliarios asociados.',
        'leads.phone_hint': 'Formato: +52 (México), +1 (EE.UU./Canadá), o 10 dígitos',
      }
  ...
  const validatePhone = (phone: string): { valid: boolean; error?: string } => {
    ...
    if (cleaned.startsWith('+52')) {
      ...
      if (digits.length === 10) {
        ...
        return { valid: false, error: 'Phone number is too long. Please include country code (+52, +1) or use 10 digits' }
      } else {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-md rounded-lg bg-white shadow-xl my-auto max-h-[90vh] overflow-y-auto"
          ...
          {success ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">
                {t2('leads.success_message', 'We\'ll connect you with a local expert soon.')}
              </p>
            </div>
          ) : (
            <>
              ...
              <p className="text-gray-600 mb-6">
                {t2('leads.form_description', 'Connect with a vetted real estate professional in your area.')}
              </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder={t2('leads.name_placeholder', 'Your full name')}
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder={t2('leads.email_placeholder', 'your@email.com')}
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder={t2('leads.phone_placeholder', '+52 55 1234 5678')}
                  />
                  {fieldErrors.phone && (
                  ...
                  {!fieldErrors.phone && touched.phone && formData.phone && (
                    <p className="mt-1 text-xs text-gray-500">
                      {t2('leads.phone_hint', 'Format: +52 (Mexico), +1 (US/Canada), or 10 digits')}
                    </p>
                  )}
                </div>
                ...
                <div className="mt-6">
                  <h3 className="text-sm font-semibold text-gray-900 mb-1">
                    {t2('leads.location_interest_heading', 'Which location are you interested in?')}
                  </h3>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      <SelectTrigger 
                        id="city"
                        className={touched.city && !formData.city ? 'border-red-300' : ''}
                        onClick={() => setTouched(prev => ({ ...prev, city: true }))}
                      >
                        <SelectValue placeholder={t2('leads.city_placeholder', 'Select city')} />
                      </SelectTrigger>
                      <SelectContent>
                ...
                <div>
                  ...
                  >
                    <SelectTrigger 
                      className={touched.budgetRange && !formData.budgetRange ? 'border-red-300' : ''}
                      onClick={() => setTouched(prev => ({ ...prev, budgetRange: true }))}
                    >
                      <SelectValue placeholder={t2('leads.budget_placeholder', 'Select budget range')} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="under-2m">{t2('leads.budget_under_2m', 'Under $2M MXN')}</SelectItem>
                      <SelectItem value="2m-5m">{t2('leads.budget_2m_5m', '$2M - $5M MXN')}</SelectItem>
                      <SelectItem value="5m-10m">{t2('leads.budget_5m_10m', '$5M - $10M MXN')}</SelectItem>
                      <SelectItem value="10m-20m">{t2('leads.budget_10m_20m', '$10M - $20M MXN')}</SelectItem>
                      <SelectItem value="over-20m">{t2('leads.budget_over_20m', 'Over $20M MXN')}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div>
                  ...
                  >
                    <SelectTrigger 
                      className={touched.timeline && !formData.timeline ? 'border-red-300' : ''}
                      onClick={() => setTouched(prev => ({ ...prev, timeline: true }))}
                    >
                      <SelectValue placeholder={t2('leads.timeline_placeholder', 'When are you planning to buy?')} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="immediately">{t2('leads.timeline_immediately', 'Immediately (0-1 months)')}</SelectItem>
                      <SelectItem value="1-3-months">{t2('leads.timeline_1_3_months', '1-3 months')}</SelectItem>
                      <SelectItem value="3-6-months">{t2('leads.timeline_3_6_months', '3-6 months')}</SelectItem>
                      <SelectItem value="6-12-months">{t2('leads.timeline_6_12_months', '6-12 months')}</SelectItem>
                      <SelectItem value="exploring">{t2('leads.timeline_exploring', 'Just exploring')}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <p className="text-xs text-gray-500 text-center">
                  {t2('leads.consent_text', 'By submitting, you agree to be contacted by our partner real estate professionals.')}
                </p>
              </form>
            </>
          )}
